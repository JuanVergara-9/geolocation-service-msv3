...
// CORS: libre en dev, whitelist en prod desde CORS_ORIGINS
...
// Health / Readiness (dejamos alias /healthz y /readyz también)
app.get('/health', (_req, res) => res.json({ ok: true, service: 'geolocation-service' }));
app.get('/ready',  async (_req, res) => {
  try { await sequelize.authenticate(); return res.json({ ok: true }); }
  catch { return res.status(503).json({ ok: false }); }
});
app.get('/healthz', (_req, res) => res.json({ ok: true, service: 'geolocation-service' }));
...
app.listen(PORT, '0.0.0.0', () => console.log(`geolocation-service on :${PORT}`));
